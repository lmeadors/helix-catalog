...
- **`@path`** 🟡 — appears on blocks that sit between a split and its join; `0` = path A, `1` = path B. Blocks on parallel A/B paths can share the same `@position` value (position appears to number *slots* in the chain, not a single linear order). `scripts/parse_hlx.py` now sorts on `(@position, @path)` and tags shared-position blocks with their A/B letter. Seen in `Matchless (v3).hlx`, where dsp0 splits post-amp into two parallel cabs and dsp1 splits into two parallel reverbs.
  - Open: the split/join blocks' *own* `@position` values don't follow a consistent rule across the two observed cases — in dsp0, `join`'s position (8) was one past the parallel row (7); in dsp1, `join`'s position (5) coincided with a block on the longer branch (`HD2_ReverbParticle`, also position 5). The diagram in `presets/samples/Matchless (v3).flow.md` was built by trusting each audible block's own `@position`/`@path` and ignoring split/join's position — **not yet verified against the actual HX Edit routing for dsp1's Path 2** (Delay → Chorus → split → [Plateaux → 63 Spring → Particle] on path A vs. [Plateaux only] on path B → join → EQ). Check next time at the device/HX Edit.
